...
import { Plus, Link as LinkIcon, ExternalLink, Check, X, User, Calendar } from "lucide-react";
          ...
          <div key={t.id} onClick={() => setDetailTask(t)} className="bg-surface border border-border p-4 rounded-xl flex items-center justify-between hover:border-primary/50 cursor-pointer transition-all group">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 flex items-center gap-1"><Calendar size={12}/> {t.due_date}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
      ...
      {isOpen && (
        <Modal title="Assign New Task" onClose={() => setIsOpen(false)}>
          <form onSubmit={handleAdd} className="space-y-4">
            ...
            <div><label className="text-xs text-gray-400 font-bold">Description</label><textarea name="description" required className="w-full bg-background border border-border rounded p-2 text-white mt-1 h-24" placeholder="Describe the task..." /></div>
        ...
        <Modal title="Task Details" onClose={() => setDetailTask(null)}>
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-2 bg-white/5 p-3 rounded-lg border border-white/5">{detailTask.description || "No description provided."}</p>
            </div>
            
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <User size={16} /> Assigned to: <span className="text-white font-bold">{detailTask.profiles?.full_name}</span>
            </div>
            ...
            <div className="border-t border-border pt-4">
              ...
              {detailTask.proof_url ? (
                ...
                <form onSubmit={handleSubmitProof} className="flex gap-2">
                    <input value={proofLink} onChange={e => setProofLink(e.target.value)} placeholder="Paste link (Drive/Video)..." className="flex-1 bg-background border border-border rounded p-2 text-sm text-white" required />
