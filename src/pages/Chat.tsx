        ...
        <div className="flex-1 flex flex-col bg-background/30 relative">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {messages.map((m, idx) => {
              ...
              const senderName = m.profiles?.full_name || 'Unknown User';
              const senderAvatar = m.profiles?.avatar_url || 'https://i.pravatar.cc/150?u=unknown';
              const isMe = senderName === 'Admin User' || senderName === 'You';

              return (
                <div key={idx} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : ''}`}>
                  <img src={senderAvatar} className="h-8 w-8 rounded-full border border-border" />
